...
fn generate_tauri_properties(
  ...
  dbg!("ENTER `generate_tauri_properties`");
  ...
  if !app_tauri_properties.is_empty() {
    ...
    let app_tauri_properties_content = dbg!(format!(
      "// THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY.\n{}",
      app_tauri_properties.join("\n")
    ));
    if std::fs::read_to_string(dbg!(&app_tauri_properties_path))
  ...
  dbg!("EXIT SUCCESS `generate_tauri_properties`");
  Ok(())
}
